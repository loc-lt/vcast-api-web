...
def openlwm():
    ...
    dmc_products_list = [row.dmc_product for row in cursor.fetchall()]
    print(dmc_products_list)
    file_list = dmc_products_list
    return render_template('pages/lwm.html',file_list=dmc_products_list)
...
def lwm(filename):
    ...
    x_axis = [float(val) for val in (data[0] + data[1]).split(';')]   
    list_a = [float(val) for val in (data[2] + data[3]).split(';')]
    list_b = [float(val) for val in (data[4] + data[5]).split(';')]
    list_c = [float(val) for val in (data[6] + data[7]).split(';')]
    list_d = [float(val) for val in (data[8] + data[9]).split(';')]
    ...
    for i in range(len(x_axis)):
        tolerance = list_a[i] * tolerance_percent
        tolerance1 = list_b[i] * tolerance_percent
        tolerance2 = list_c[i] * tolerance_percent
        tolerance3 = list_d[i] * tolerance_percent
        if count>jitter_percent:
            lower_bound = list_a[i] - tolerance
            upper_bound = list_a[i] + tolerance
            lower_bound1 = list_b[i] - tolerance1
            upper_bound1 = list_b[i] + tolerance1
            lower_bound2 = list_c[i] - tolerance2
            upper_bound2 = list_c[i] + tolerance2
            lower_bound3 = list_d[i] - tolerance3
            upper_bound3 = list_d[i] + tolerance3
            count=0
    ...
    #plt.show()
    html_fig = mpld3.fig_to_html(figure)
        ...
        if check_secr(24): 
            if request.method == 'POST':
                ...
                if soluong >0:
                    ...
                    ax1 = df1.plot(kind='bar', y='Count', figsize=(20, 10), color=['darkgreen', 'red'], width = 0.7)
        ...
        if check_secr(28): 
            if request.method == 'POST':
                ...
                # html_data = data.to_html(classes='table table-striped',table_id="example",border=0,index=False)
                    ...
                    plt.xlabel('Time')
                    plt.ylabel('Quantity')
                    plt.title('Biểu đồ cột của Quantity theo thời gian')
    ...
    if request.cookies.get("login_status")=='ok':
        ...
        if check_secr(12): 
            if request.method == 'POST':
                ...
                plt.scatter(df1.index, df1['average_temp1'], label='Average Temperature',  s=5)
                plt.scatter(df1.index, df1['average_humid1'], label='Average Humidity',  s=5)
                plt.xlabel('Datetime')
                plt.ylabel('Values')
                plt.title('Average Temperature & Humidity',loc='left')
                plt.legend(bbox_to_anchor= (1, 1.08), ncol = 2)
                plt.xticks(rotation=45, fontsize= 8)
                plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d %H:%M'))  # Định dạng lại đồng tử trục x
                plt.tight_layout()
                plt.savefig('static/images/Coating1.png')
                #plt.show()
                return render_template('pages/Coating.html',user=user,process=process,post_flag = True,data = data,page=1,soluong=soluong,soluong1=soluong1,timestart=timestart,timeend=timeend)
            else:
...
def download_temp_coating():
    ...
    timeend=request.form['timeend']
    #print(timestart)
    df1 = pd.read_sql("select * from data_Coating1_W5 where Datetime >'"+timestart+"' and Datetime <'"+timeend+"'", conn)
    df2 = pd.read_sql("select * from data_Coating2_W5 where Datetime >'"+timestart+"' and Datetime <'"+timeend+"'", conn)
    df3 = pd.read_sql("select * from data_Coating3_W5 where Datetime >'"+timestart+"' and Datetime <'"+timeend+"'", conn)
    df4 = pd.read_sql("select * from data_Coating4_W5 where Datetime >'"+timestart+"' and Datetime <'"+timeend+"'", conn)
    df5 = pd.read_sql("select * from data_Coating5_W5 where Datetime >'"+timestart+"' and Datetime <'"+timeend+"'", conn)
    df6 = pd.read_sql("select * from data_Coating6_W5 where Datetime >'"+timestart+"' and Datetime <'"+timeend+"'", conn)
    output = io.BytesIO()
    writer = pd.ExcelWriter(
        output,
        engine='xlsxwriter')    # add a sheet
...
def heat2(filename):
    ...
    return jsonify(data)
@app.route("/HeatTreatment/realtime", methods=['GET'])
def open_HeatTreatmentrt_area():
    if request.cookies.get("login_status")=='ok':
        user = request.cookies.get("user_name")
        process="Heat Treatment"
        if check_secr(2): 
            process="Heat Treatment"
            return render_template('/pages/HeatTreatment.html',user=user,process=process)
        else:
            return render_template('pages/error.html',user=user)
        
    else:
        return redirect(url_for('login'))
@app.route("/HeatTreatment/realtime/<area>/get", methods=['POST'])
def chart_HeatTreatmentrt(area):
    if request.cookies.get("login_status")=='ok':
        user = request.cookies.get("user_name")
        process="HeatTreatment"
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT TOP (100) * FROM [SPC].[dbo].[HXL_HeatTreatment_Data] where machine = '192.168.0.103' order by id desc" )
        data =cursor.fetchall()
        value = []
        value2 = []
        value3 = []
        ...
        money = []
        for i in range(len(data)):
            value.append(round(data[i][3]))
            value2.append(round(data[i][4]))
            value3.append(round(data[i][5]))
            money.append(round(data[i][5]))
            time.append(int(data[i][2].timestamp()*1000+25200000))
        data = {}
        # time_from_sql = [datetime.datetime.strptime(str(t), '%Y-%m-%d %H:%M:%S') for t in time]
    ...
    try:
        ...
        for idmc in range(len(dmclist)):
            try:
                ...
                data = pd.read_sql("SELECT [DMC],[Line],[TimeSave],[id],[actual],[Result],[CodePurpose] FROM [QC].[dbo].[CMMdata] where dmc = '"+dmclist[idmc]+"'",conn)
